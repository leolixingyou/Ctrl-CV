...
class CarlaAckermannControl(CompatibleNode):
    ...
    def __init__(self):
        ...
        self.control_loop_rate = self.get_param("control_loop_rate", 0.00001)
    ...
    def run_speed_control_loop(self):
        ...
        ### xingyou: self.info.restrictions -> vehicle info from self.vehicle_info_updated
    ...
    def update_current_values(self):
        ...
        :return:
        """
        ### xingyou: self.get_time()-> rospy.get_time()즉 현재 ROS 시간을 가져오기 
        ### xingyou: self.info.current.time_sec-> self.get_time() 즉 초기화 했을때 ROS 시간을 가져오기 
        current_time_sec = self.get_time()
        delta_time = current_time_sec - self.info.current.time_sec
        ### xingyou: self.vehicle_status from callback self.vehicle_status_updated
        current_speed = self.vehicle_status.velocity
        if delta_time > 0:
        ### xingyou: updated speed info -> self.info.current.speed default 0
    ...
    def run_manual(self):
        ...
        while not rospy.is_shutdown():
            ...
            def loop(timer_event=None):
                    self.last_ackermann_msg_received_sec = self.get_time()
                    # set target values
                    self.set_target_steering_angle(self.target_steer) # desired virtual angle (radians)
                    # one more param # desired rate of change (radians/s)
                    self.set_target_speed(self.target_speed) # desired forward speed (m/s)
                    self.set_target_accel(self.target_acc) # desired acceleration (m/s^2)
                    self.set_target_jerk(self.target_jerk) # desired jerk (m/s^3)
                    self.update_current_values()
                    self.vehicle_control_cycle()
                    self.send_ego_vehicle_control_info_msg()
            if key != -1:
