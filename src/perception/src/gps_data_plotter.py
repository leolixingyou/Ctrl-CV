...
def plot_gps_data(latitudes, longitudes):
    ...
    ax.plot(latitudes, longitudes, 'r-', linewidth=2, markersize=6)
    ax.plot(latitudes[0], longitudes[0], 'go', markersize=10, label='Start')
    ax.plot(latitudes[-1], longitudes[-1], 'bo', markersize=10, label='End')
    ...
    def on_click(event):
        if event.inaxes == ax:
            ...
            if event.button == 1:  # Left click
                selected_points.append(( nearest_lat, nearest_lon))
                marker, = ax.plot( nearest_lat, nearest_lon, 'yo', markersize=10)
                point_markers.append(marker)
                print(f"Point selected: {nearest_lat}, {nearest_lon}")
            elif event.button == 3:  # Right click
                for i, (lat, lon ) in enumerate(selected_points):
                    if (lat, lon ) == ( nearest_lat, nearest_lon):
                        selected_points.pop(i)
                        point_markers[i].remove()
                        point_markers.pop(i)
                        print(f"Point removed: {lat}, {lon}")
                        break
            fig.canvas.draw()
...
if __name__ == "__main__":
    ...
    input_file_path = '/workspace/src/perception/src/gps_log/odm_x_y_yaw_abs_log_12.txt'
