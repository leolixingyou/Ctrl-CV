...
# ==============================================================================
# -- My_CarlaSpawnObjects ------------------------------------------------------------
# ==============================================================================


class My_CarlaSpawnObjects(CompatibleNode):
    ...
    def clean_up_ego_vehicle(self,object_id):
        try:
            destroy_object_request = roscomp.get_service_request(DestroyObject)
            destroy_object_request.id = object_id
            self.call_service(self.destroy_object_service,
                        destroy_object_request, timeout=0.5, spin_until_response_received=True)
...
class Actor_Listener:
    ...
    def get_id(self,):
        if self.get_info:
            return [x.id for x in self.msg.actors if x.rolename == 'ego_vehicle']

    def destory_obj(self,):
        obj_temp_id = self.get_id()
