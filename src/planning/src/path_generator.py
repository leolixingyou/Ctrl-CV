...
def get_fromplanning_course(dl, path):
    ax, ay = reformatting(path)
...
def load_arrays_from_file():
    arrays = []
    filename='/workspace/src/planning/src/array_data_globalmap_utm.txt'
...
def plot_path_and_direction(cx, cy, cyaw):
...
def save_path_data(cx, cy, cyaw, ck, filename='/workspace/src/planning/src/path_data.txt'):
    data = np.column_stack((cx, cy, cyaw, ck))
    np.savetxt(filename, data, header='X,Y,Yaw,Curvature', delimiter=',', comments='')
    print(f"Data saved to {filename}")

class Path_Generator:
    def __init__(self) -> None:
        self.check_point_condition = False
    ...
    def run(self, path):
        ...
        start = time.time()
        dl = 1.0  # course tick
        cx, cy, cyaw, ck = get_fromplanning_course(dl, path)
        return cx, cy, cyaw, ck

if __name__ == '__main__':

    path_g = Path_Generator()
    # waypoints = path_g.read_waypoints()
    waypoints = load_arrays_from_file()
    cx, cy, cyaw, ck = path_g.run(waypoints)
    save_path_data(cx, cy, cyaw, ck)

    plot_path_and_direction(cx, cy, cyaw)